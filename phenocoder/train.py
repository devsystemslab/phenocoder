...
from phenocoder.generator import setup_generators
import yaml
...
def train_model(
    ...
    generator_train, generator_val, df_cond, oh_enc = setup_generators(
        dir_dataset,
        conditional,
    ...
    if conditional:
        cvae = CondCVAE(
            n_classes=generator_train.conditions.shape[-1],
    ...
    figure_reconstructions = plot_reconstructions(
        cvae, generator_train, n_preview=300, return_fig=True, show=False
    )
    with file_writer.as_default():
        tf.summary.image(
            'input vs reconstruction', plot_to_image(figure_reconstructions), step=0
        )
